...
class State(object):
    ...
    def updateState(self):
        ...
        if self.ball_x >= 1 and self.ball_y >= self.paddle_y and self.ball_y <= self.paddle_y + PADDLE_HEIGHT:
            ...
            # randomize the speed
            while True:
                self.velocity_x = -self.velocity_x + uniform(-0.015/2, 0.015/2)
                if abs(self.velocity_x) > 0.03/2:
                    break
...
class stupidSunJiaRun(object):
    ...
    def initAgainstHuman(self):
        pygame.init()
        self.clock = pygame.time.Clock()
        self.screen = pygame.display.set_mode(SCREEN_SIZE)
        self.total_competition = 0
        self.human_winnings = 0

        pygame.display.set_caption('Simple Game')


    def playMode(self):
        self.initAgainstHuman()
        ...
        while True:
            if self.state.endOfGame():
                self.total_competition += 1
                print("Game over!")
                if self.state.isWin():
                    print("AI wins!")
                else:
                    print("Human wins!")
                    self.human_winnings += 1
                self.state.reset()
                print("playing game %d times, human winnings rate is %f." % (self.total_competition, self.human_winnings/float(self.total_competition)))

            else:
                currState = self.state.discretizeState()
                (ball_x, ball_y, _, _, paddle_y, paddle_y2) = currState
                ...
                if bar_move_up == True and bar_move_down == False:
                    human_action = MOVE_DOWN

                if bar_move_down == True and bar_move_up == False:
                    human_action = MOVE_UP
                
                if bar_move_down == False and bar_move_up == False:
                    human_action = NOTHING

                self.screen.fill(BLACK)

                self.state.movePaddle(currAction, PLAYER1)
                self.state.movePaddle(human_action, PLAYER2)
                reward = self.state.updateState()

                paddle1_center_y = (1 - self.state.paddle_y - PADDLE_HEIGHT) * 400
                paddle2_center_y = (1 - self.state.player2_y - PADDLE_HEIGHT) * 400
                
                paddle1_pos = pygame.Rect(400 - BAR_SIZE[0], paddle1_center_y, BAR_SIZE[0], BAR_SIZE[1])
                paddle2_pos = pygame.Rect(0, paddle2_center_y, BAR_SIZE[0], BAR_SIZE[1])
                ...
                pygame.draw.rect(self.screen, WHITE, paddle2_pos)
...
if __name__ == "__main__":
    ...
    game = stupidSunJiaRun(2, INITIAL_EPSILON, FINAL_EPSILON, 0.1)
    game.train(100000)
    game.test()
    game.playMode()
