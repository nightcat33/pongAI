...
class Agent(object):
    ...
    def eplison_decay(self):
        if self.curr_epsilon > self.final_epsilon:
            self.curr_epsilon -= (self.init_epsilon - self.final_epsilon) / float(EXPLORE)
...
class stupidSunJiaRun(object):
    ...
    def train(self, episodes):
        ...
        with open('single_train_size20_results.csv', 'a') as f:
            writer = csv.writer(f)
            writer.writerow(row)
    ...
    def test(self):
        ...
        with open('single_size20_test_results.csv', 'a') as f:
            writer = csv.writer(f)
            writer.writerow(row)
...
if __name__ == "__main__":
    ...
    # game = stupidSunJiaRun(20, INITIAL_EPSILON, FINAL_EPSILON, 0.4)
    # game.train(100000)
    # game.test()
    # game.initGUI()
    # game.playMode()
